...
class Mesh:
    ...
    def create_mesh(self, revers_time=False):
        ...
        self.result = (self.start.nodes, self.left.nodes, self.right.nodes, self.center.nodes, self.finish.nodes)
...
class PartMesh(ABC):
    ...
    @abstractmethod
    def get_array_node(self) -> List[Node]:
        pass
...
class MeshStart(PartMesh):
    ...
    def connect(self):
        for i in range(1, len(self.nodes)):
            self.nodes[i].left = self.nodes[i-1] if self.inds[i-1] == 1. else None
            self.nodes[i-1].right = self.nodes[i] if self.inds[i-1] == 0. else None
...
class MeshCenter(PartMesh):
    ...
    def create(self, start_mesh, left_mesh):
        ...
        start_nodes = start_mesh.nodes[1:-1]
        self.nodes = [[NoneCenter(node.s, node.t+self.dt*i) for node in start_nodes] for i in range(1, count_level+1)]
        self.connect()
...
class MeshLeft(PartMesh):
    ...
    def create(self, start_mesh):
        start_node = start_mesh.nodes[0]
        dt = self.h[0]+self.h[1]
        t = np.arange(start_node.t+dt, self.t1-dt, dt)
        self.nodes = [NodeLeft(self.s0, ti) for ti in t]
        self.connect()
